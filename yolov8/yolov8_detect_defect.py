...
model = YOLO("weights/defect13_v8l_60e/weights/best.pt")
# Define yolov8 classes
cls_names = ['Dent','PinHole','Gas','Slag','Shrinkage','SandDrop','SandBroken','Other']

img_lists = glob.glob('images/test/*.jpg')

for img_path in img_lists:
    img_name = os.path.split(img_path)[-1]

    v8bbox, v8conf, v8class ,img = yolov8_detect(model,img_path)
    img = draw_detections(img,v8bbox,v8conf,v8class)
    cv2.namedWindow("Detected Objects", cv2.WINDOW_NORMAL)
    cv2.imshow("Detected Objects", img)
    cv2.imwrite("images/out/"+img_name, img)
    cv2.waitKey()
